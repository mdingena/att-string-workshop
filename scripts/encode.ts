...
const resolvedFileURL = pathToFileURL(join(dirname(currentPath), '..', filePath));

/**
 * ⚠️ DO NOT EDIT THIS FILE ⚠️
 */

if (!existsSync(resolvedFileURL)) {
  console.error(`Could not find creation file: ./${resolvedFileURL.toString()}`);
  process.exit(2);
}
...
const module = await import(resolvedFileURL.toString());
...
writeFileSync('./encoded.txt', creation.toSaveString());
console.log(
  `\nEncoding "${importKey}" from "./${filePath}".\nResults have also been saved in "./encoded.txt".\n`
);
creation.print();
